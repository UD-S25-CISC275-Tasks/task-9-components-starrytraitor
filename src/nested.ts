...
export function findQuestion(
    ...
    let q: Question | undefined = questions.find(
...
export function sameType(questions: Question[]): boolean {
    ...
    let first: QuestionType = questions[0].type;
    return questions.every((question: Question) => question.type === first);
}
    ...
    return questions.map((question: Question) => {
        if (question.id === targetId) {
            if (targetOptionIndex === -1) {
                return addOption(question, newOption);
            } else {
                let q: Question = {
                    ...question,
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce(
        (arr: Question[], question: Question): Question[] => {
            arr.push(question);
            if (question.id === targetId) {
                arr.push(duplicateQuestion(newId, question));
            }
            return arr;
        },
        [],
